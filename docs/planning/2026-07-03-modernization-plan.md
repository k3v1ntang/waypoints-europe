...
**Status**: Phases 1–5 merged and on-device verified (2026-07-04/05). Only Phase 6 (Banff + PMTiles, post-trip) remains. Merge freeze 2026-08-14.
...
- **Tint bias (user decision, 2026-07-04)**: bias all glass toward the *more tinted/frosted* end of the spectrum — higher fill opacity, stronger blur — not maximum transparency. Rationale: at WWDC June 2026 Apple itself reduced Liquid Glass's default transparency and reworked its foundations "to ensure exceptional readability" for iOS 27; the tinted end is where the language has settled and it ages better over a live map, which is busier than most app content.
...
### D9. iOS 26 standalone viewport rule: `position: fixed`, never absolute-to-document (found Session E, 2026-07-05)

Diagnosed on-device with an instrumented overlay: iOS 26 sizes a standalone (installed-PWA) web app's **document containing block to screen-minus-top-inset** (e.g. `html.clientHeight` 812) while the **fixed-position viewport is the full screen** (`innerHeight` 874); `env(safe-area-inset-*)` reports correctly. Any `position: absolute` surface anchored to the document therefore sits ~62px above the true viewport bottom — the cause of Session E's black-band/floating-attribution/overlap symptom cluster. Fix shipped (`1047c6f`): map container and Map banners are `position: fixed`, so every surface shares one coordinate system; `env()`-based layout retained, no measurement hacks. **Rule: viewport-anchored elements in this app must use `position: fixed`.** Phase 6's download-management UI must follow this.
...
### Phase 5 — UX modernization: interaction restructure + Liquid Glass (two stages) ✅ (5a merged 2026-07-05; 5b merged + on-device verified 2026-07-05)
...
- **2026-07-06 — Docs sync (unlettered) — merged.** PR #7 → `a94d69e`; commit `e758f93`; worker model Sonnet 5. Stale architecture/implementation docs updated to the post-modernization reality (history preserved in technology-decisions.md); two new durable docs distilled from this plan and the session notes: `docs/architecture/design-system.md` (tokens, glass rules, tint bias, z-ladder, D9) and `docs/implementation/operations-runbook.md` (deploy/rollback, CI gates, offline caching, PWA update path, edit export flow, watch items). Orchestrator verified factual claims against tokens.css/App.tsx/git tags before merge. Also: `npm audit` re-run post-upgrades (0 vulnerabilities), no injection sinks in src/ — next security review is triggered by Phase 6's new surfaces or the share-with-others milestone, not the calendar. Discoverability fix (2026-07-06, per July 2026 progressive-disclosure best practice, refined by user): status split across two new files — `/MEMORY.md` at the repo root (mutable now-snapshot: active project, state, freeze, open items; updated in the same commit by whichever session changes status) and `docs/planning/README.md` (descriptive only: how the planning system works). CLAUDE.md carries only durable pointers to both plus project-independent rules — volatile status never lives in CLAUDE.md or README.md, and history lives only in plan Session Logs, so each fact has exactly one home.
- **2026-07-05 — Docs cleanup (unlettered, post-Phase-5) — merged.** PR #6 → `8e1614b`; commit `e4ad0dd`; worker model Sonnet 5. CLAUDE.md synced to the TypeScript/tokens reality, dead `src/config/theme.js` deleted (orchestrator-verified zero imports), stale tokens.css comment fixed. Gates green. Repo is now in its pre-trip resting state; remaining Session E deferred findings wait for post-trip.
- **2026-07-05 — Session E (Phase 5b) — merged + on-device verified.** PR #5 → `b1ba3dd`, then a post-merge hotfix chain `ca00fb7`→`f2f778b` on main (each CI-green and user-verified on the installed production PWA); worker model Fable 5; rollback tag `pre-E-stable` = `3049513`. All Phase 5 success criteria met: two-surface consolidation (CityNavigation/POISearch/FABs deleted), Export ≤2 taps, zero plain-JSX components (lint now 0 warnings — the Session B downgrades died with the rewritten files), 60fps accepted de facto, reduced-transparency/motion mechanisms verified via emulation. *Gate deviation (Session B precedent)*: merged on user decision before an HTTPS preview pass existed — Netlify deploy previews have been disabled since PR #2; production was the test surface with `pre-E-stable` as the net. *Major finding promoted to D9*: iOS 26 standalone document-vs-viewport mismatch; `position: fixed` rule. *Dormant Phase 1 bugs found & fixed*: cluster-tap zoom (MapLibre v4 removed `getClusterExpansionZoom` callback API), map-label font 404s ("DIN Offc Pro" → Noto Sans), MapLibre ctrl glass never applied (CSS load-order specificity). *Deviations*: search sheet is a full-screen opaque takeover (D4 small-surfaces + iOS keyboard); editor sheet intentionally has no grab handle (drafts protected); Positron not adopted. *Open spot-checks (non-blocking)*: airplane-mode-across-restart on the new build, iOS Reduce Transparency/Motion toggles, edit-export sanity. *Follow-ups for a future cleanup session*: CLAUDE.md structure/key-files stale (deleted `.jsx` files, dead `theme.js`, stale tokens.css comment); unify 3 sheet shells into one primitive; `ITINERARY_ORDER` lives in SearchSheet.tsx (Amsterdam/Paris data-contract relevance); `:has()` coupling to MapLibre internals; duplicate text-normalization; scheme-aware theme-color vs. blue manifest (brand consistency); re-enable Netlify deploy previews before the next session gate.
- **2026-07-05 — Session D (Phase 5a + PoC) — merged.** PR #4 → `17e6bef` on main; commits `353835e` (tokens/glass/BottomBar), `bb80e43` (review fixes), `74a1f37` + `3628b86` (user-driven iterations); worker model Fable 5. **PoC hard-stop gate passed**: user approved the tinted-glass material and bottom bar on iPhone + Mac (2026-07-04, two iteration rounds). Objective gates re-verified by orchestrator (tsc/lint/tests/build). Deviations documented in `docs/planning/2026-07-04-session-d-notes.md` (old FABs repositioned for comparison; interim search popover, opaque per D4; tsconfig extended to components for born-`.tsx`). */code-review*: 10 findings fixed; 4 of 8 finder agents rate-limited and completed inline by the worker — clean rerun deliberately skipped since Session E rewrites these surfaces and reviews them again. **Comparison period is live in production** (both control generations visible; known iOS keyboard-overlap quirk on the new search field) until Session E. *Carried to E*: keyboard overlap, duplicated search logic (`POISearch.jsx` deletion pending), dark-mode mixed look, `prefers-reduced-transparency` fallback and production-PWA smoke unverified on-device.
...
> You are the orchestration session for docs/planning/2026-07-03-modernization-plan.md — read /MEMORY.md (repo root) first for current state, then this plan fully; §5's orchestration rules define your role. You advise and edit the plan, /MEMORY.md, and my persistent memory only; you never write application code. Each turn: verify actual repo state (git log, plan status marks, CI) before trusting anything I paste, and check the current git branch before committing — worker sessions share this checkout. When I give you a worker's Session Report or an on-device gate result, judge it against that phase's success criteria and lead with a verdict — merge, fix (give me the exact follow-up prompt for a worker session), or rollback (name the tag). Tell me when to close the current worker session, which model to open the next one with, and give me its exact cold-start prompt. After each merged session, update the plan's status marks and Session Log plus the /MEMORY.md snapshot, and commit that doc-only change. Keep replies short: verdict first, reasoning second. Start by reading /MEMORY.md, the plan, and current git state, then tell me the current status and the next action with its prompt.

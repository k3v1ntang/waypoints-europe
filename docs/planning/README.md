...
Describes the planning system for this repo. For **current status**, read [MEMORY.md](MEMORY.md) — this file is descriptive and changes rarely.

## Structure

- **`MEMORY.md`** — the mutable now-snapshot: active project, one-line state, freeze dates, open items. The only file here that tracks status. Maintained by the active project's orchestrator session; replaced on update (history lives in each plan's Session Log, never here).
